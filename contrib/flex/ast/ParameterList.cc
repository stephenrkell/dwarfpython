...
ParathonValue *ParameterList::evaluate(ParathonContext& c)
{
    return NULL;
}
...
std::vector<ParathonValue*>* ParameterList::asArgs(ParathonContext& c)
{
    std::vector<ParathonValue*> *a = new std::vector<ParathonValue*>;
    for (unsigned i=0; i < this->params.size(); i++)
    {
...
bool ParameterList::assign(ParathonContext &c, std::vector<ParathonValue*> *values)
{
